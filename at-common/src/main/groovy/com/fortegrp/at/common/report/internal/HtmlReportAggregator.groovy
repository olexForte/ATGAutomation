...
class HtmlReportAggregator extends AbstractHtmlCreator<Map> {
    ...
    void aggregateReport(String specName, Map stats, String outputDir) {
        ...
        for (int i in 0..20) {
            if (lockedReportFile.exists()) {
                log.warn(String.format("Report generator is busy. Wait for %s second...", (i + 1)))
                sleep(1000)
            } else {
                break
            }
        ...
        try {
            ...
            if (new File(reportsDir.path + "/index.html.lck").exists())
            {
                new File(reportsDir.path + "/index.html.lck").delete()
            }
        } catch (e) {
            if (new File(reportsDir.path + "/index.html.lck").exists())
            {
    ...
    public Map recomputeAggregateData() {
    ...
    protected void writeDetails(MarkupBuilder builder, Map ignored, String resultSuffix, String appendResult) {
        ...
        builder.table(['style': ReportStylesheet.styleMap.get('table.summary-table'), 'class': 'sortable']) {
            thead {
                ...
                th('style': ReportStylesheet.styleMap.get('table.summary-table th'), 'Environment')
            ...
            tbody {
                div('id': 'results_' + resultSuffix) {
                    aggregatedData.keySet().sort().each { String specName ->
                        ...
                        def testPackageName = specName.substring(0, specName.lastIndexOf(".")).replace("com.ff.at.rest.specs", "c.ff.a.r.s")
