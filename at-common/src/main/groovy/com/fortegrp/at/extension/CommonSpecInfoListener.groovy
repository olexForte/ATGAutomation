...
@Slf4j
class CommonSpecInfoListener implements IRunListener {

    final HtmlReportCreator reportCreator
    SpecData specData
    ...
    CommonSpecInfoListener(HtmlReportCreator reportCreator) {
    ...
    synchronized void beforeSpec(SpecInfo spec) {
        log.info("[Fork #${CommonEnvironment.forkNumber}] Specification execution (${spec.name})")
    ...
    void beforeFeature(FeatureInfo feature) {
        log.info("[Fork #${CommonEnvironment.forkNumber}] Feature execution (${feature.name})")
        feature.blocks.each {
            log.info("[Fork #${CommonEnvironment.forkNumber}] " + it.getKind().toString() + ((it.getTexts().size() > 0) ? " " + it.texts : ""))
        }
    ...
    void afterFeature(FeatureInfo feature) {
        ...
        log.info "[Fork #${CommonEnvironment.forkNumber}] Feature is finished (${feature.name})"
    }

    @Override
    void afterSpec(SpecInfo spec) {
        log.info "[Fork #${CommonEnvironment.forkNumber}] Specification is finished (${spec.name})"
    ...
    void error(ErrorInfo error) {
        log.error("[Fork #${CommonEnvironment.forkNumber}] ", error.exception)
    ...
    void specSkipped(SpecInfo spec) {
        log.info "[Fork #${CommonEnvironment.forkNumber}] Specification is skipped (${spec.name})"
    }

    @Override
    void featureSkipped(FeatureInfo feature) {
        log.info "[Fork #${CommonEnvironment.forkNumber}] Feature is skipped (${feature.name})"
    }
